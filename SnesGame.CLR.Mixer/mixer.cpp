...
namespace SnesGame {
    namespace CLR {
            ...
            static void WriteNoteToChannel(Note^ note, hSC sc) {
                ...
                if (squareNote != nullptr) {
                    playNote_SC(sc, squareNote->MaxLength,
                        squareNote->VolumeMin, squareNote->VolumeMax, (ShiftDir)squareNote->VolumeShiftParameters->Direction,
                        squareNote->VolumeShiftParameters->Speed, (::EdgeBehaviour)squareNote->VolumeShiftParameters->EdgeBehaviour,
                        squareNote->PeriodMin, squareNote->PeriodMax, (ShiftDir)squareNote->PeriodShiftParameters->Direction,
                        squareNote->PeriodShiftParameters->Speed, (::EdgeBehaviour)squareNote->PeriodShiftParameters->EdgeBehaviour);
                }

                Noise^ noise = dynamic_cast<Noise^>(note);
                if (noise != nullptr) {
                    playNoise_SC(sc, noise->InitialRegister, noise->TapMask, noise->MaxLength, noise->Speed);
                }
            }
        ...
        public:
            ...
            static array<Int16>^ SampleAudio(ReadOnlyRecording^ recording, int startIndex, int endIndex)
            {
