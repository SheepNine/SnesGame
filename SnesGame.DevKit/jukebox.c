...
hRecord creat_Record(Uint8 trackCount, Uint8* opcodeStream,
    Uint16* trackOffsets, Uint16* trackLengths)
{
        ...
        else
        {
            ...
            while (track->opcodeStreamPos < track->opcodeStreamLength)
            {
                ...
                case OPC_DEFAULT_PERIOD_BOTH:
                    track->singleNoteParams.periodLow
                        = track->opcodeStream[track->opcodeStreamPos++] << 8;
                    track->singleNoteParams.periodLow
                        += track->opcodeStream[track->opcodeStreamPos++];
                    track->singleNoteParams.periodHigh
                        = track->singleNoteParams.periodLow;
                    track->defaultParams.periodLow
                        = track->singleNoteParams.periodLow;
                    track->defaultParams.periodHigh
                        = track->singleNoteParams.periodHigh;
                    break;
                ...
                case OPC_DEFAULT_VOLUME_BOTH:
                    track->singleNoteParams.volumeLow
                        = track->opcodeStream[track->opcodeStreamPos++];
                    track->singleNoteParams.volumeHigh
                        = track->singleNoteParams.volumeLow;
                    track->defaultParams.volumeLow
                        = track->singleNoteParams.volumeLow;
                    track->defaultParams.volumeHigh
                        = track->singleNoteParams.volumeHigh;
                    break;
                ...
                case OPC_DEFAULT_LENGTH:
                    track->singleNoteParams.length
                        = track->opcodeStream[track->opcodeStreamPos++] << 8;
                    track->singleNoteParams.length
                        += track->opcodeStream[track->opcodeStreamPos++];
                    track->defaultParams.length
                        = track->singleNoteParams.length;
                    break;
                case OPC_DEFAULT_VSHIFT_SPEED:
                    track->singleNoteParams.volumeShift.speed
                        = track->opcodeStream[track->opcodeStreamPos++];
                    track->defaultParams.volumeShift.speed
                        = track->singleNoteParams.volumeShift.speed;
                    break;
                case OPC_DEFAULT_PSHIFT_SPEED:
                    track->singleNoteParams.periodShift.speed
                        = track->opcodeStream[track->opcodeStreamPos++];
                    track->defaultParams.periodShift.speed
                        = track->singleNoteParams.periodShift.speed;
                    break;
                case OPC_DEFAULT_VSHIFT_EB:
                    ...
                    track->defaultParams.volumeShift.edgeBehaviour
                        = track->singleNoteParams.volumeShift.edgeBehaviour;
                    break;
                case OPC_DEFAULT_PSHIFT_EB:
                    ...
                    track->defaultParams.periodShift.edgeBehaviour
                        = track->singleNoteParams.periodShift.edgeBehaviour;
                    break;
                case OPC_DEFAULT_VSHIFT_DIR:
                    ...
                    track->defaultParams.volumeShift.dir
                        = track->singleNoteParams.volumeShift.dir;
                    break;
                case OPC_DEFAULT_PSHIFT_DIR:
                    ...
                    track->defaultParams.periodShift.dir
                        = track->singleNoteParams.periodShift.dir;
                    break;
                case OPC_DEFAULT_NOISE_LENGTH:
                    track->singleNoteNoiseParams.maxLength
                        = track->opcodeStream[track->opcodeStreamPos++] << 8;
                    track->singleNoteNoiseParams.maxLength
                        += track->opcodeStream[track->opcodeStreamPos++];
                    track->defaultNoiseParams.maxLength
                        = track->singleNoteNoiseParams.maxLength;
                    break;
                case OPC_DEFAULT_NOISE_REGISTER:
                    track->singleNoteNoiseParams.initialRegister
                        = track->opcodeStream[track->opcodeStreamPos++] << 8;
                    track->singleNoteNoiseParams.initialRegister
                        += track->opcodeStream[track->opcodeStreamPos++];
                    track->defaultNoiseParams.initialRegister
                        = track->singleNoteNoiseParams.initialRegister;
                    break;
                case OPC_DEFAULT_NOISE_TAP:
                    track->singleNoteNoiseParams.tapBit
                        = track->opcodeStream[track->opcodeStreamPos++] << 8;
                    track->singleNoteNoiseParams.tapBit
                        += track->opcodeStream[track->opcodeStreamPos++];
                    track->defaultNoiseParams.tapBit
                        = track->singleNoteNoiseParams.tapBit;
                    break;
                case OPC_DEFAULT_NOISE_REPEAT:
                    track->singleNoteNoiseParams.period
                        = track->opcodeStream[track->opcodeStreamPos++];
                    track->defaultNoiseParams.period
                        = track->singleNoteNoiseParams.period;
                    break;
