    ...
    Uint8 bankIndex, Uint8 paletteIndex, SDL_bool mask0)
{
    while (x < 32 && (*string) != 0)
    {
        setBackdropStroke(render, backdropIndex, x++, y, *(string++),
            bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, mask0);
    }
}
    ...
    Uint8 bankIndex, Uint8 paletteIndex, SDL_bool hFlip, SDL_bool vFlip, SDL_bool mask0)
{
    for (Uint8 y = 0; y < 32; y++)
    {
        ...
        {
            setBackdropStroke(render, backdropIndex, x, y, brushIndex,
