...
typedef struct Dialog {
	...
	char* text; // Copied, massaged
	Uint16 numLinesInText; // one per line break
	Uint16* lineStartIndices; // index of first character of line
	Uint16* lineEndIndices; // index of last character of line, starts 1 before its lineStart, advances until it is after its lineStart and 1 before a line end

	Uint16 firstVisibleLine;
	Uint8 textScrollOffset; // 0..7 (0..15 for double-high text); when it exceeds range, topLine ticks up and scrollOffset resets
	SDL_bool tallText;
...
SDL_bool pageDone_Dialog(hDialog dialog) {
	...
	if (dialog->firstVisibleLine % dialog->numLinesPerPage == 0 && dialog->textScrollOffset == 0 || dialog->firstVisibleLine + dialog->numLinesPerPage == dialog->numLinesInText) {
		return SDL_TRUE;
	}
	else {
		return SDL_FALSE;
...
void renderText_Dialog(hDialog dialog, hRENDER render, Uint8 layerIndex, Uint8 bankIndex, Uint8 paletteIndex) {
	...
	for (Uint16 line = dialog->firstVisibleLine; line < dialog->numLinesInText; line++) {
		Uint8 destY = dialog->pageTop + (dialog->tallText ? 2 : 1) * (Uint8)(line - dialog->firstVisibleLine) - dialog->textScrollOffset / 8;
		Uint8 destX = dialog->pageLeft;

		if (destY > 31) break;

		for (int c = dialog->lineStartIndices[line]; c < dialog->lineEndIndices[line]; c++) {
			setBackdropStroke(render, layerIndex, destX, destY, dialog->text[c], bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
			if (dialog->tallText) {
				setBackdropStroke(render, layerIndex, destX, destY + 1, dialog->text[c] + 128, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
			}
			destX++;
		}
	}
...
void renderBorder_Dialog(hDialog dialog, hRENDER render, Uint8 layerIndex, Uint8 bankIndex, Uint8 paletteIndex) {
	...
	fillBackdrop(render, layerIndex, 0, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_TRUE);

	setBackdropStroke(render, layerIndex, 0, 29 - dialog->numLinesPerPage, 1, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
	setBackdropStroke(render, layerIndex, 30, 29 - dialog->numLinesPerPage, 3, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
	setBackdropStroke(render, layerIndex, 0, 30, 6, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
	setBackdropStroke(render, layerIndex, 30, 30, 8, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);

	for (Uint8 x = 1; x < 30; x++) {
		setBackdropStroke(render, layerIndex, x, 29 - dialog->numLinesPerPage, 2, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
		setBackdropStroke(render, layerIndex, x, 30, 7, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
	}

	for (Uint8 y = 30 - dialog->numLinesPerPage; y < 30; y++) {
		setBackdropStroke(render, layerIndex, 0, y, 4, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
		setBackdropStroke(render, layerIndex, 30, y, 5, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
	}

	if (pageDone_Dialog(dialog) && !isFinished_Dialog(dialog)) {
		setBackdropStroke(render, layerIndex, 29, 30, 9, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
	}
	...
	if (dialog->portraitWB != NULL) {
		...
		if (dialog->pageLeft == 1) {
			...
			appendToBackdrop_WB(render, layerIndex, dialog->portraitWB, 30 - portraitWidth, 25, bankIndex, paletteIndex, SDL_FALSE);
		}
		else {
			dividerX = 1 + portraitWidth;
			appendToBackdrop_WB(render, layerIndex, dialog->portraitWB, 1, 25, bankIndex, paletteIndex, SDL_FALSE);
		}

		setBackdropStroke(render, layerIndex, dividerX, 29 - dialog->numLinesPerPage, 10, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
		setBackdropStroke(render, layerIndex, dividerX, 30, 12, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
		for (Uint8 y = 30 - dialog->numLinesPerPage; y < 30; y++) {
			setBackdropStroke(render, layerIndex, dividerX, y, 11, bankIndex, paletteIndex, SDL_FALSE, SDL_FALSE, SDL_FALSE);
		}
	}
}
