            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                case CV_SQUARE: {
                    ...
                    playNote_SC(soundChannels[i], params.length,
                        ...
                        params.volumeShift.dir,
                        params.volumeShift.speed,
                        params.volumeShift.edgeBehaviour,
                        params.periodLow, params.periodHigh,
                        params.periodShift.dir,
                        params.periodShift.speed,
                        params.periodShift.edgeBehaviour);
                    break;
                }
                case CV_NOISE: {
                    NoiseParams params = voiceBuffer[i].waveParams.noise;
                    playNoise_SC(soundChannels[i], params.initialRegister,
                        params.tapBit, params.maxLength, params.period);
                    break;
                }
                }
            }

            for (int i = 0; i < voiceBufferHead - 8; i++)
            { // SKETCHY: not copying all of them
...
extern int libMain(char* title,
    pInitCallback initFunc, pUpdateCallback updateFunc, pRenderCallback renderFunc,
    pDeinitCallback deinitFunc, SDL_RWops* recordStream, SDL_RWops* playbackStream)
{
    int result = 0;
    bb = creat_BB();
