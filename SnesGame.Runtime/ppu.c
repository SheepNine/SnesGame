...
{
    ppu->debugBackdropEnable[layerIndex]
        = (ppu->debugBackdropEnable[layerIndex] == SDL_TRUE) ? SDL_FALSE : SDL_TRUE;
}

void debugToggleActor_PPU(hPPU ppu, Uint8 layerIndex)
{
    ppu->debugActorEnable[layerIndex]
        = (ppu->debugActorEnable[layerIndex] == SDL_TRUE) ? SDL_FALSE : SDL_TRUE;
}
...
void _packStroke_PPU(Uint8* stroke, Uint8 brushIndex, Uint8 bankIndex, Uint8 paletteIndex,
    SDL_bool hFlip, SDL_bool vFlip, SDL_bool mask0)
{
    stroke[0] = (mask0 ? 0x80 : 0x00) | (hFlip ? 0x40 : 0x00) | (vFlip ? 0x20 : 0x00)
        | (paletteIndex << 2) | (bankIndex);
    stroke[1] = brushIndex;
}

void _unpackStroke_PPU(Uint8* stroke, Uint8* brushIndex, Uint8* bankIndex, Uint8* paletteIndex,
    SDL_bool* hFlip, SDL_bool* vFlip, SDL_bool* mask0)
{
...
void _packBackdropControl_PPU(Uint8* control, Uint8 dX, Uint8 dY, SDL_bool visible,
    SDL_bool topmost)
{
...
void _setPaletteColor_PPU(Uint8* palettes, Uint8 paletteIndex, Uint8 colorIndex,
    Uint8 r, Uint8 g, Uint8 b, SDL_bool t)
{
    _packColor_PPU(palettes + 32 * paletteIndex + 2 * colorIndex, r, g, b, t);
}

void setBackdropPaletteColor_PPU(hPPU ppu, Uint8 paletteIndex, Uint8 colorIndex,
    Uint8 r, Uint8 g, Uint8 b, SDL_bool t)
{
    _setPaletteColor_PPU(ppu->backdropPalettes, paletteIndex, colorIndex, r, g, b, t);
}

void setActorPaletteColor_PPU(hPPU ppu, Uint8 paletteIndex, Uint8 colorIndex,
    Uint8 r, Uint8 g, Uint8 b, SDL_bool t)
{
...
void setBackdropStroke_PPU(hPPU ppu, Uint8 backdropIndex, Uint8 x, Uint8 y,
    Uint8 brushIndex, Uint8 bankIndex, Uint8 paletteIndex,
    SDL_bool hFlip, SDL_bool vFlip, SDL_bool mask0)
{
    _packStroke_PPU(ppu->backdropStrokes + 2 * (32 * (32 * backdropIndex + y) + x),
        brushIndex, bankIndex, paletteIndex, hFlip, vFlip, mask0);
}

void setBackdropControl_PPU(hPPU ppu, Uint8 backdropIndex, Uint8 dX, Uint8 dY,
    SDL_bool visible, SDL_bool topmost)
{
    _packBackdropControl_PPU(ppu->backdropControls + backdropIndex, dX, dY, visible, topmost);
}

void _unpackHClips_PPU(Uint8* clipSet,
    Uint8* leftWidth, Uint8* rightWidth, SDL_bool* invertLeft, SDL_bool* invertRight)
{
...
void _unpackVClips_PPU(Uint8* clipSet,
    Uint8* topWidth, Uint8* bottomWidth, SDL_bool* invertTop, SDL_bool* invertBottom)
{
...
void _packClips_PPU(Uint8* clipSet,
...
void setFullLayerClips_PPU(hPPU ppu, Uint8 layerIndex,
    Uint8 leftWidth, Uint8 rightWidth, Uint8 topHeight, Uint8 bottomHeight,
    SDL_bool invertLeft, SDL_bool invertRight, SDL_bool invertTop, SDL_bool invertBottom)
{
    _packClips_PPU(ppu->layerClips + 4 * layerIndex,
        leftWidth, rightWidth, topHeight, bottomHeight,
        invertLeft, invertRight, invertTop, invertBottom);
}

void setLayerClips_PPU(hPPU ppu, Uint8 layerIndex,
    Uint8 leftWidth, Uint8 rightWidth, Uint8 topHeight, Uint8 bottomHeight)
{
    setFullLayerClips_PPU(ppu, layerIndex, leftWidth, rightWidth, topHeight, bottomHeight,
        SDL_FALSE, SDL_FALSE, SDL_FALSE, SDL_FALSE);
}

void setActorStroke_PPU(hPPU ppu, Uint8 actorIndex, Uint8 brushIndex,
    Uint8 bankIndex, Uint8 paletteIndex, SDL_bool hFlip, SDL_bool vFlip, SDL_bool mask0)
{
    _packStroke_PPU(ppu->actorStrokes + 2 * actorIndex, brushIndex,
        bankIndex, paletteIndex, hFlip, vFlip, mask0);
}
...
void _packActorControl_PPU(Uint8* control, int x, int y, Uint8 sizeX, Uint8 sizeY,
    SDL_bool visible, Uint8 layer)
{
    ...
    control[0] = xByte;
    control[1] = yByte;
    control[2] = (invertX ? 0x80 : 0x00) | (invertY ? 0x40 : 0x00)
        | (layer << 4) | (visible ? 0x08 : 0x00) | (size);
}
...
void setActorControl_PPU(hPPU ppu, Uint8 actorIndex, int x, int y, Uint8 sizeX, Uint8 sizeY,
    SDL_bool visible, Uint8 layer)
{
    _packActorControl_PPU(ppu->actorControls + 3 * actorIndex, x, y,
        sizeX, sizeY, visible, layer);
}
...
void _scanBackdrop(Uint8* control, Uint8* backdrop, hMapper mapper, Uint8* palettes, hSL sl)
{
    ...
    for (int i = 0; i < 32; i++)
    {
        ...
        scanBar_SL(sl, getBackgroundBar_Mapper(
            mapper, bankIndex, brushIndex, vFlip ? 7 - glyphBarIndex : glyphBarIndex),
...
void _scanBackdrop_PPU(hPPU ppu, Uint8 layerIndex, hSL sl, SDL_bool topmost)
{
    ...
    {
        _scanBackdrop(control, ppu->backdropStrokes + 2048 * layerIndex,
            ppu->romMapper, ppu->backdropPalettes, sl);
    }
}
